...
export default function CommandPalette({ open, user, onClose, onNavigate, onResearch }: CommandPaletteProps) {
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-[80] flex items-start justify-center bg-[#10201d]/55 px-4 pt-[12vh] backdrop-blur-sm" onMouseDown={onClose}><div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Command palette" onMouseDown={(event) => event.stopPropagation()} className="w-full max-w-2xl overflow-hidden rounded-2xl border border-white/20 bg-[var(--card)] shadow-[0_30px_80px_rgba(5,15,30,.35)]"><div className="flex items-center gap-3 border-b border-[var(--border)] px-5"><Search size={18} className="text-[var(--ink-soft)]" /><input autoFocus value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Search commands, agents and workflows…" aria-label="Search commands" className="h-16 min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-[var(--ink-soft)]" /><button onClick={onClose} aria-label="Close command palette" className="rounded-lg p-2 text-[var(--ink-soft)] hover:bg-[var(--hover)]"><X size={17} /></button></div><div className="max-h-[430px] overflow-auto p-2"><p className="px-3 pb-2 pt-2 text-[10px] font-semibold text-[var(--ink-soft)]">Available commands</p>{shown.map((item) => <button key={item.label} onClick={() => { item.run(); onClose(); }} className="group flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left hover:bg-[var(--hover)]"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--hover)] text-[var(--accent)]"><item.icon size={18} /></span><span className="min-w-0 flex-1"><span className="block text-sm font-medium text-[var(--ink)]">{item.label}</span><span className="mt-0.5 block truncate text-xs text-[var(--ink-soft)]">{item.detail}</span></span>{item.shortcut && <kbd className="rounded-md border border-[var(--border)] bg-[var(--card)] px-2 py-1 text-[10px] font-medium text-[var(--ink-soft)] shadow-sm">{item.shortcut}</kbd>}</button>)}{!shown.length && <div className="px-4 py-12 text-center text-sm text-[var(--ink-soft)]">No matching command</div>}</div><div className="flex items-center justify-between border-t border-[var(--border)] bg-[var(--card)] px-5 py-3 text-[10px] text-[var(--ink-soft)]"><span>Role scope: <strong className="capitalize text-[var(--ink-soft)]">{user.role}</strong></span><span>Esc to close / ⌘K to toggle</span></div></div></div>;
}
