...
export default function AdminWorkspace() {
  ...
  return <section className="mx-auto max-w-[1240px] px-5 py-7 md:px-8 md:py-9">
    ...
    <div className="panel mt-6 overflow-hidden"><div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-4"><div><h3 className="text-sm font-semibold">Recent audit activity</h3><p className="mt-1 text-xs text-[var(--ink-soft)]">Append-only operational evidence trail</p></div><Activity size={18} className="text-[var(--accent)]"/></div><div className="divide-y divide-[var(--border)]">{events.slice(0, 12).map((event) => <div key={event.id} className="grid gap-1 px-5 py-3 text-xs sm:grid-cols-[1fr_1fr_auto]"><span className="font-medium text-[var(--ink)]">{event.action}</span><span className="text-[var(--ink-soft)]">{event.actor_name ?? "System"} / {event.resource_type}</span><time className="text-[var(--ink-soft)]">{new Date(event.timestamp).toLocaleString()}</time></div>)}</div></div>
  </section>;
}
