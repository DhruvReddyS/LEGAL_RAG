...
from app.ingestion.chunker import chunk_structural_units
...
def intake_response(
    intake: CorpusIntake, stored: StorageObject
) -> CorpusIntakeResponse:
...
async def update_professional_user(
    ...
    *,
    ...
    if target.role not in {UserRole.POLICE, UserRole.ADVOCATE}:
        raise HTTPException(
            status_code=403, detail="Only police and advocate accounts are managed here"
        )
    previous = {"role": target.role.value, "is_active": target.is_active}
    if role is not None and role != target.role:
        case_count = await session.scalar(
            select(func.count(Case.id)).where(Case.owner_id == target.id)
        )
        if case_count:
            raise HTTPException(
                status_code=409,
                detail="Role cannot change while the account owns cases",
            )
        target.role = role
...
async def stage_corpus_intake(
    ...
    *,
    ...
    if not content or len(content) > MAX_CORPUS_UPLOAD_BYTES:
        raise HTTPException(
            status_code=413, detail="Corpus documents must be between 1 byte and 30 MiB"
        )
    if content_type != "application/pdf" and not filename.lower().endswith(".pdf"):
        raise HTTPException(
            status_code=415, detail="Global corpus intake currently accepts PDF only"
        )
    if not content.startswith(b"%PDF-"):
        raise HTTPException(
            status_code=422, detail="File content is not a valid PDF header"
        )
    if not source_url.startswith("https://"):
        raise HTTPException(
            status_code=422, detail="An HTTPS official source URL is required"
        )
    ...
    if duplicate is not None:
        raise HTTPException(
            status_code=409, detail="This exact document is already staged or indexed"
        )
...
async def validate_corpus_intake(
    ...
    *,
    ...
    if intake.status == "published":
        raise HTTPException(
            status_code=409, detail="Published corpus intake cannot be revalidated"
        )
    ...
    content = await DocumentStorageService(session).download_case_document(
        stored.id, admin
    )
...
async def publish_corpus_intake(
    ...
    *,
    ...
    if intake.status != "validated":
        raise HTTPException(
            status_code=409, detail="Only a validated intake can be published"
        )
    stored = await session.get(StorageObject, intake.storage_object_id)
    if stored is None:
        raise HTTPException(status_code=409, detail="Corpus object is missing")
    ...
    extracted = ExtractedDocument(
        ...
        source_path=stored.original_filename,
        pages=pages,
    )
    document = CanonicalDocument(
        ...
        local_path=stored.object_key,
        source_type=intake.source_type.value,
        category="admin_extended",
        authority=intake.authority,
        jurisdiction=intake.jurisdiction,
        court=intake.authority
        if intake.source_type is CorpusSourceType.JUDGMENT
        else None,
        act_name=(
            intake.title
            if intake.source_type
            in {CorpusSourceType.ACT, CorpusSourceType.NOTIFICATION}
            else None
        ),
    ...
    all_chunks = chunk_structural_units(document, units)
    rejected_chunks = [chunk for chunk in all_chunks if chunk.quality != "indexed"]
    chunks = [chunk for chunk in all_chunks if chunk.quality == "indexed"]
    if not chunks:
        raise HTTPException(
            status_code=422, detail="Validated document produced no indexable chunks"
        )
    # BGE pads every batch to its longest member. Unlike the resumable gold
    # pipeline, this one-shot publisher can group similar lengths safely.
    chunks.sort(key=lambda chunk: len((chunk.embed_text or chunk.text).split()))
    embeddings = await retrieval.embed_documents(
        [chunk.embed_text or chunk.text for chunk in chunks], batch_size=8
    )
    points = [
        models.PointStruct(
            ...
            payload={
                **legal_chunk_payload(chunk, corpus_tier="extended"),
                "source_url": intake.source_url,
            },
        )
    ...
    source = (
        await session.get(CorpusSource, intake.corpus_source_id)
        if intake.corpus_source_id
        else None
    )
    if source is None:
        source = CorpusSource(
    ...
    intake.validation_summary = {
        **intake.validation_summary,
        ...
        "rejected_noise_chunks": len(rejected_chunks),
...
async def admin_overview(session: AsyncSession) -> AdminOverviewResponse:
    async def count_where(model, *conditions) -> int:
        return int(
            await session.scalar(select(func.count(model.id)).where(*conditions)) or 0
        )

    return AdminOverviewResponse(
        users_total=await count_where(User),
        police_active=await count_where(
            User, User.role == UserRole.POLICE, User.is_active.is_(True)
        ),
        advocates_active=await count_where(
            User, User.role == UserRole.ADVOCATE, User.is_active.is_(True)
        ),
        staged_intakes=await count_where(CorpusIntake, CorpusIntake.status == "staged"),
        validated_intakes=await count_where(
            CorpusIntake, CorpusIntake.status == "validated"
        ),
        published_intakes=await count_where(
            CorpusIntake, CorpusIntake.status == "published"
        ),
